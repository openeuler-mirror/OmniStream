...
<!-- md-trans-meta sourceCommit=731f32ad32aa4150677f561ef77f51e17015c69a translatedAt=2026-09-15T07:49:49.667Z pushedAt=2026-09-15T08:18:53.478Z -->
...
<tr id="row4474639131117"><td class="cellrowborder" valign="top" width="50%" headers="mcps1.1.3.1.1 "><p id="p647453931110"><a name="p647453931110"></a><a name="p647453931110"></a><a href="https://atomgit.com/openeuler/libboundscheck.git">libboundscheck</a></p>
</td>
...
During source code compilation, build libboundscheck as a shared library, enable
...
OmniStream searches `$HOME/opt/lib` for OmniRuntime dynamic libraries and
`$HOME/OmniOperatorJIT` for OmniOperator headers by default. Source code compilation dependencies, KACC JSON, and the LLVM 15 runtime library can be installed in `/usr/local`.

Configure OmniOperator before compiling OmniStream to generate `core/config.h`.
...
In the following examples, `/opt/build/OmniStream` is the source directory.
Replace it with the actual clone directory.

1. Before compiling OmniStream, compile the OmniAdaptor code.
...
Run the unit test after completing the compilation.
...
2. After starting sql-client, perform the test.
...
This code repository plans to participate in the open-sourcing of Flink software, solely for Flink feature extension and performance improvement. The coding style follows the native open-source software, inherits the security design of the native open-source software, and does not undermine the design, coding style, and methods of the native open-source software. Any vulnerabilities and security issues of the software shall be resolved by the corresponding upstream communities according to their vulnerability and security response mechanisms. Please pay attention to the notifications and version updates released by the upstream communities. The Kunpeng computing community does not assume any responsibility for software vulnerabilities and security issues.
