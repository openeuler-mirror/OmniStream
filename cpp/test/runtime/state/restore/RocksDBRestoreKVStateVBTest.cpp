...
TEST_F(RocksDBRestoreKVStateVBTest, EmptyComboIdListIsStoredAsEmptyValue)
{
    omnistream::RocksDBWriterContext context{db_, 2 * 1024 * 1024, 1, 0, -1, nullptr, nullptr};
    const std::vector<int8_t> key{5, 4, 3};

    omnistream::RocksDBRestoreKVStateVB<int> writer(
        context, mainCf_, vbCf_, 0, {omniruntime::type::DataTypeId::OMNI_LONG}, 1024);
    writer.writeComboIdList(key, {});
    writer.flush();

    std::string restoredValue = "not-empty";
    auto status = db_->Get(
        rocksdb::ReadOptions(),
        mainCf_,
        rocksdb::Slice(reinterpret_cast<const char*>(key.data()), key.size()),
        &restoredValue);
    ASSERT_TRUE(status.ok()) << status.ToString();
    EXPECT_TRUE(restoredValue.empty());
}
