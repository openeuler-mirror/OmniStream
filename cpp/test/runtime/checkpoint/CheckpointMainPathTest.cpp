...
public:
    int64_t GetHeaderLength() const override { return 100; }
};
...
public:
    ...
    std::shared_ptr<BufferRecycler> GetRecycler() override { return DummyObjectBufferRecycler::getInstance(); }
    void RecycleBuffer() override { recycled = true; }
    bool IsRecycled() const override { return recycled; }
    Buffer* RetainBuffer() override { return this; }
    Buffer* ReadOnlySlice() override { return this; }
    Buffer* ReadOnlySlice(int, int) override { return this; }
    int GetMaxCapacity() const override { return 512; }
    ...
    int ReadableObjects() const override { return 1; }
    ...
    std::string ToDebugString(bool) const override { return "MockBuffer"; }
    ObjectSegment* GetObjectSegment() override { return nullptr; }
    int GetBufferType() override { return 0; }
    std::pair<uint8_t*, size_t> GetBytes() override { return {nullptr, 0}; }
    bool recycled = false;
};
...
TEST(CheckpointInProgressRequestTest, ConstructorWithAction) {
    CheckpointInProgressRequest req("Test", JobVertexID(1, 1), 0, 42,
...
TEST(CheckpointInProgressRequestTest, ExecuteTransitionsState) {
    CheckpointInProgressRequest req("Test", JobVertexID(1, 1), 0, 1,
        [](std::shared_ptr<ChannelStateCheckpointWriter>&) {});
...
TEST(ChannelStateWriteRequestDispatcherImplTest, AbortedCheckpointIdDetection) {
    ...
    auto abortReq = ChannelStateWriteRequest::terminate(jvid, 1, 5,
        std::make_exception_ptr(std::runtime_error("abort")));
...
TEST(ChannelStateWriteRequestDispatcherImplTest, NoStartBeforeAbortDoesNotThrow) {
    ...
    auto abortReq = ChannelStateWriteRequest::terminate(jvid, 1, 1,
        std::make_exception_ptr(std::runtime_error("early abort")));
    EXPECT_NO_THROW(dispatcher->dispatch(abortReq));
}
...
public:
    ...
    void ConvertToPriorityEvent(int, int) override { prioritized = true; }
    ...
    int GetInputGateIndex() override { return gateIdx; }
...
TEST(BarrierHandlerStateTest, SingleChannelBarrierReceivedTriggersCheckpoint) {
    ...
    auto options = std::make_shared<CheckpointOptions>(CheckpointType::CHECKPOINT,
...
TEST(ChannelStateWriteResultTest, IsDoneAfterBothFuturesComplete) {
    ...
    auto outputFuture = std::make_shared<CompletableFutureV2<
...
TEST(ChannelStateCheckpointWriterTest, ConstructorRequiresNonEmptySubtasks) {
    ...
    EXPECT_THROW({
        ChannelStateCheckpointWriter writer(empty, 1, nullptr,
