...
TEST(GroupAggSavepointAdaptorTest, ConvertHeapMapStateEmitsNullAndNonNullEntries)
{
    GroupAggSavepointAdaptor adaptor;
    IntSerializer keySerializer;
    IntSerializer valueSerializer;
    int key1 = 3;
    int value1 = 30;
    int key2 = 4;
    DataOutputSerializer heapValue(64);
    heapValue.writeInt(2);
    keySerializer.serialize(&key1, heapValue);
    heapValue.writeBoolean(false);
    valueSerializer.serialize(&value1, heapValue);
    keySerializer.serialize(&key2, heapValue);
    heapValue.writeBoolean(true);
    auto heapBytes = copyOutput(heapValue);

    VectorBatchSaveStateContext context;
    context.writable = true;
    context.mappedKvStateId = 2;
    context.logicalStateName = DISTINCT_STATE_NAME;
    context.stateType = VectorBatchStateType::KV_MAP_TRANSFORM;
    context.mapKeySerializer = &keySerializer;
    context.mapValueSerializer = &valueSerializer;
    const std::vector<int8_t> sourceKey{7, 8};
    KeyValueStateIterator::CurrentEntry entry;
    entry.key = ByteView(sourceKey.data(), sourceKey.size());
    entry.value = ByteView(heapBytes.data(), heapBytes.size());

    std::vector<ConvertedEntry> converted;
    adaptor.convertKVRowData(
        entry, context, VectorBatchSavePlan{}, [&](ConvertedEntry result) { converted.push_back(std::move(result)); });
    ASSERT_EQ(converted.size(), 2U);
    EXPECT_EQ(converted[0].context, &context);
    EXPECT_TRUE(std::equal(sourceKey.begin(), sourceKey.end(), converted[0].keyBytes.begin()));

    DataInputDeserializer firstKeyInput(
        reinterpret_cast<const uint8_t*>(converted[0].keyBytes.data()),
        static_cast<int>(converted[0].keyBytes.size()),
        static_cast<int>(sourceKey.size()));
    std::unique_ptr<int> decodedKey1(static_cast<int*>(keySerializer.deserialize(firstKeyInput)));
    ASSERT_NE(decodedKey1, nullptr);
    EXPECT_EQ(*decodedKey1, key1);
    DataInputDeserializer firstValueInput(
        reinterpret_cast<const uint8_t*>(converted[0].valueBytes.data()),
        static_cast<int>(converted[0].valueBytes.size()),
        0);
    EXPECT_FALSE(firstValueInput.readBoolean());
    std::unique_ptr<int> decodedValue1(static_cast<int*>(valueSerializer.deserialize(firstValueInput)));
    ASSERT_NE(decodedValue1, nullptr);
    EXPECT_EQ(*decodedValue1, value1);

    DataInputDeserializer secondKeyInput(
        reinterpret_cast<const uint8_t*>(converted[1].keyBytes.data()),
        static_cast<int>(converted[1].keyBytes.size()),
        static_cast<int>(sourceKey.size()));
    std::unique_ptr<int> decodedKey2(static_cast<int*>(keySerializer.deserialize(secondKeyInput)));
    ASSERT_NE(decodedKey2, nullptr);
    EXPECT_EQ(*decodedKey2, key2);
    DataInputDeserializer secondValueInput(
        reinterpret_cast<const uint8_t*>(converted[1].valueBytes.data()),
        static_cast<int>(converted[1].valueBytes.size()),
        0);
    EXPECT_TRUE(secondValueInput.readBoolean());
    EXPECT_EQ(secondValueInput.Available(), 0);
}

TEST(GroupAggSavepointAdaptorTest, ConvertHeapMapStateHandlesEmptyMapAndRejectsBadFraming)
{
    GroupAggSavepointAdaptor adaptor;
    IntSerializer keySerializer;
    IntSerializer valueSerializer;
    ...
    context.mapKeySerializer = &keySerializer;
    context.mapValueSerializer = &valueSerializer;
    const std::vector<int8_t> key{1};
    KeyValueStateIterator::CurrentEntry entry;
    entry.key = ByteView(key.data(), key.size());
    size_t outputCount = 0;

    DataOutputSerializer emptyMap;
    emptyMap.writeInt(0);
    auto bytes = copyOutput(emptyMap);
    entry.value = ByteView(bytes.data(), bytes.size());
    EXPECT_NO_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [&](ConvertedEntry) { ++outputCount; }));
    EXPECT_EQ(outputCount, 0U);

    bytes = {0, 0, 0};
    entry.value = ByteView(bytes.data(), bytes.size());
    EXPECT_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [](ConvertedEntry) {}), std::runtime_error);

    DataOutputSerializer negativeSize;
    negativeSize.writeInt(static_cast<uint32_t>(-1));
    bytes = copyOutput(negativeSize);
    entry.value = ByteView(bytes.data(), bytes.size());
    EXPECT_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [](ConvertedEntry) {}), std::runtime_error);

    DataOutputSerializer trailingBytes;
    trailingBytes.writeInt(0);
    trailingBytes.writeByte(0x7f);
    bytes = copyOutput(trailingBytes);
    entry.value = ByteView(bytes.data(), bytes.size());
    EXPECT_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [](ConvertedEntry) {}), std::runtime_error);
}

TEST(GroupAggSavepointAdaptorTest, ConvertHeapMapStateRejectsMissingOrFailingSerializers)
{
    GroupAggSavepointAdaptor adaptor;
    ...
    DataOutputSerializer oneEntry;
    oneEntry.writeInt(1);
    int mapKey = 9;
    intSerializer.serialize(&mapKey, oneEntry);
    oneEntry.writeBoolean(false);
    auto bytes = copyOutput(oneEntry);
    entry.value = ByteView(bytes.data(), bytes.size());

    context.mapKeySerializer = nullptr;
    context.mapValueSerializer = &intSerializer;
    EXPECT_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [](ConvertedEntry) {}), std::runtime_error);
    context.mapKeySerializer = &intSerializer;
    context.mapValueSerializer = nullptr;
    EXPECT_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [](ConvertedEntry) {}), std::runtime_error);

    context.mapKeySerializer = &nullSerializer;
    context.mapValueSerializer = &intSerializer;
    EXPECT_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [](ConvertedEntry) {}), std::runtime_error);

    context.mapKeySerializer = &intSerializer;
    context.mapValueSerializer = &nullSerializer;
    EXPECT_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [](ConvertedEntry) {}), std::runtime_error);
}

TEST(GroupAggSavepointAdaptorTest, ConvertAccumulatorExpandsRawPositionAndPreservesKey)
{
    GroupAggSavepointAdaptor adaptor;
    adaptor.prepareForSave(GROUP_AGG_DESCRIPTION);
    omnistream::RowType omniRowType(true, std::vector<std::string>{"BIGINT", "INT", "VARCHAR"});
    RowDataSerializer omniSerializer(&omniRowType);
    std::unique_ptr<BinaryRowData> omniAccumulator(BinaryRowData::createBinaryRowDataWithMem(3));
    omniAccumulator->setLong(0, 2002);
    omniAccumulator->setInt(1, 23);
    omniAccumulator->setStringView(2, "expanded");
    auto sourceBytes = serializeRow(omniSerializer, *omniAccumulator);

    omnistream::RowType flinkRowType(true, std::vector<std::string>{"BIGINT", RAW_TYPE, "INT", "VARCHAR"});
    RowDataSerializer flinkSerializer(&flinkRowType);
    VectorBatchSaveStateContext context;
    context.logicalStateName = ACC_STATE_NAME;
    context.stateType = VectorBatchStateType::KV_TRANSFORM;
    context.sourceValueSerializer = &omniSerializer;
    context.valueSerializer = &flinkSerializer;
    const std::vector<int8_t> key{4, 5, 6};
    KeyValueStateIterator::CurrentEntry entry;
    entry.key = ByteView(key.data(), key.size());
    entry.value = ByteView(sourceBytes.data(), sourceBytes.size());
    ConvertedEntry converted;

    adaptor.convertKVRowData(
        entry, context, VectorBatchSavePlan{}, [&](ConvertedEntry result) { converted = std::move(result); });

    EXPECT_EQ(converted.context, &context);
    EXPECT_EQ(converted.keyBytes, key);
    DataInputDeserializer input(
        reinterpret_cast<const uint8_t*>(converted.valueBytes.data()),
        static_cast<int>(converted.valueBytes.size()),
        0);
    auto* expanded = static_cast<BinaryRowData*>(flinkSerializer.deserialize(input));
    ASSERT_NE(expanded, nullptr);
    EXPECT_EQ(*expanded->getLong(0), 2002);
    EXPECT_TRUE(expanded->isNullAt(1));
    EXPECT_EQ(*expanded->getInt(2), 23);
    EXPECT_EQ(expanded->getStringView(3), "expanded");
}

TEST(GroupAggSavepointAdaptorTest, ConvertAccumulatorRejectsInvalidContextAndDeserializeFailure)
{
    GroupAggSavepointAdaptor adaptor;
    NullDeserializingSerializer nullSerializer;
    const std::vector<int8_t> bytes{1, 2, 3, 4};
    KeyValueStateIterator::CurrentEntry entry;
    entry.key = ByteView(bytes.data(), bytes.size());
    entry.value = ByteView(bytes.data(), bytes.size());
    VectorBatchSaveStateContext context;

    context.stateType = VectorBatchStateType::KV;
    context.sourceValueSerializer = &nullSerializer;
    EXPECT_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [](ConvertedEntry) {}), std::runtime_error);
    context.stateType = VectorBatchStateType::KV_TRANSFORM;
    context.sourceValueSerializer = nullptr;
    EXPECT_THROW(
        adaptor.convertKVRowData(entry, context, VectorBatchSavePlan{}, [](ConvertedEntry) {}), std::runtime_error);
    context.sourceValueSerializer = &nullSerializer;
    EXPECT_THROW(
