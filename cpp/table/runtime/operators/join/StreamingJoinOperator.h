...
private:
        ...
        JoinRecordStateView<K>* otherSideStateView)
    {
        ...
        // (3) one row for each record on the other side that just found its first match and its null-padded entry needs
        // to be deleted
        ...
        // set the RowKind and timestamp. Only when both sides uses inner join, it uses the input's RowKind and
        // Timestamp
        setOutPutMetaData(input, inputIsOuter, otherIsOuter, outputVB);
        return outputVB;
    };

    omnistream::VectorBatch* buildOutputInner(
    ...
    template <typename T, typename S>
    omniruntime::vec::BaseVector* buildOtherSideColumn(
        omnistream::VectorBatch* input, JoinRecordStateView<K>* otherSideStateView, int32_t icol, bool inputIsOuter);

    omniruntime::vec::BaseVector* buildOtherSideColumnVarchar(
        omnistream::VectorBatch* input, JoinRecordStateView<K>* otherSideStateView, int32_t icol, bool inputIsOuter);

    void setOutPutValueInput(
        omnistream::VectorBatch* input,
        bool inputIsLeft,
        bool inputIsOuter,
        JoinRecordStateView<K>* otherSideStateView,
