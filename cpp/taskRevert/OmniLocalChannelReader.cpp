...
namespace omnistream {
    ...
    void OmniLocalChannelReader::setRowDataToPtr(RowData* binaryRowData, uint8_t* dataResultContainer, unsigned int& position, int vectorBatchCol)
    {
        ...
        DataOutputSerializer valueOutputSerializer(4);
        ...
        StreamElement streamElement_(static_cast<StreamElementTag>(0));
        streamElement_.setValue(binaryRowData);
        streamElementSerializer.serialize(&streamElement_, valueOutputSerializer);
        ...
        for (int i = offset; i < vbNum + offset; i++) {
            ...
            if (dynamic_cast<StreamRecord *>(streamElement)) {
                ...
                for (int j = 0; j < vectorBatchRow; j++) {
                    ...
                    setRowDataToPtr(binaryRowData, dataResultContainer, position, vectorBatchCol);
                }
            }
        }
