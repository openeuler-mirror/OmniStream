...
std::vector<KafkaPartitionSplit> KafkaSourceReader::snapshotState(long checkpointId)
{
    ...
    INFO_RELEASE("h30082497 KafkaSourceReader::snapshotState 1 ======== splits.size() : " + std::to_string(splits.size()));
    ...
    INFO_RELEASE("h30082497 KafkaSourceReader::snapshotState 2 ======== splits.size() : " + std::to_string(splits.size()));
    return splits;
}
