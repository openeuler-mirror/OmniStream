...
private:
    static const nlohmann::json* UnwrapStateList(const nlohmann::json* listJson)
    {
        ...
        if (listJson->is_array() && listJson->size() == 2 && listJson->at(0).is_string() &&
            listJson->at(1).is_array()) {
            return &listJson->at(1);
        }
        // Some OmniAdaptor paths serialize the collection as a plain object.
        if (listJson->is_object() && listJson->contains("stateObjects") && listJson->at("stateObjects").is_array()) {
        ...
        const nlohmann::json& description, const std::string& fieldName)
    {
        ...
        for (const auto& item : *stateList) {
            ...
            if (handle == nullptr) {
                INFO_RELEASE(
                    "Error: ParseHandleAndLocalPathList Unsupported stream state handle in "
                    "IncrementalRemoteKeyedStateHandle "
                    << fieldName);
                throw std::runtime_error(
                    "Unsupported stream state handle in IncrementalRemoteKeyedStateHandle " + fieldName);
            }
            std::string localPath = item.contains("localPath") ? item.at("localPath").get<std::string>() : "";
            if (localPath.empty()) {
                INFO_RELEASE(
                    "Error: ParseHandleAndLocalPathList IncrementalRemoteKeyedStateHandle "
                    << fieldName << " entry is missing localPath.");
                throw std::runtime_error(
                    "IncrementalRemoteKeyedStateHandle " + fieldName + " entry is missing localPath.");
            }
    ...
    static const nlohmann::json& GetHandleAndLocalPathItems(const nlohmann::json& description, const char* fieldName)
    {
