...
public:
    ...
    {
        return status.load() == STATUS_SHUTDOWN;
    }
...
private:
    ...
    public:
        ScheduledTask(std::atomic<int>& serviceStatus, ProcessingTimeCallback* callback, long timestamp, long period)
        ...
        void run()
        {
            ...
            try {
                ...
                LOG("Error: processing timer callback failed, timestamp=" << nextTimestamp << ", error=" << e.what());
            } catch (...) {
                LOG("Error: processing timer callback failed, timestamp=" << nextTimestamp << ", error=unknown");
            }
    ...
    ScheduledFutureTask* scheduleRepeatedly(
        ProcessingTimeCallback* callback, long initialDelay, long period, bool fixedDelay)
    {
        ...
        try {
            ...
            THROW_RUNTIME_ERROR(
                "failed to schedule given task, current stats of ProcessingTimeService is " +
