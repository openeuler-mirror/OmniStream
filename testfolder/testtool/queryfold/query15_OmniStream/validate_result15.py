...
flink_file = os.path.join(omni_home,"testtool/queryfold/query15_flink/query15_result_flink.txt")
omni_file = os.path.join(omni_home,"testtool/queryfold/query15_OmniStream/flink_output.txt")
task_name_file1 = os.path.join(omni_home,"testtool/queryfold/query15_flink/query15_taskname.txt")
task_name_file2 = os.path.join(omni_home,"testtool/queryfold/query15_OmniStream/q15-taskname-OmniStream.txt")
...
if check_log_for_setup_stram_task(log_file)==False:
    print("❌q15 failed, plan not triggered")
    exit(1)
...
if not os.path.exists(flink_file) or not os.path.exists(omni_file):
    print("❌ q15 failed, can not find file")
    exit()
...
#
# if not os.path.exists(flink_file) or not os.path.exists(omni_file):
#     print("❌q15 failed, the txt files were not found")
#     exit()
#
#
...
if flink_lines == omni_lines:
    ...
    print("❌q15 failed, output results are wrong")
