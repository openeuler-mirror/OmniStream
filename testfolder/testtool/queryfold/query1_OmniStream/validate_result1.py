...
flink_file = os.path.join(omni_home,"testtool/queryfold/query1_flink/query1_result_flink.txt")
omni_file = os.path.join(omni_home,"testtool/queryfold/query1_OmniStream/flink_output.txt")
task_name_file1 = os.path.join(omni_home,"testtool/queryfold/query1_flink/query1_taskname.txt")
task_name_file2 = os.path.join(omni_home,"testtool/queryfold/query1_OmniStream/q1-taskname-OmniStream.txt")
...
if check_log_for_setup_stram_task(log_file)==False:
    print("❌q1 failed, plan not triggered")
    exit(1)
...
def normalize_line(line):
    ...
    if len(parts) >= 4:
        try:
            # 尝试将第3个字段（index 2）转换为 float 并格式化
...
if not os.path.exists(flink_file) or not os.path.exists(omni_file):
    print("❌q1 failed, the txt files were not found")
    exit(1)
...
#     print("❌q1 failed, native tasks do not match")

if flink_lines == omni_lines:
    print("✅q1 success")
else:
    print("❌q1 failed, output results are wrong")
    for i, (f_line, o_line) in enumerate(zip(flink_lines, omni_lines)):
        if f_line != o_line:
    ...
    if len(flink_lines) != len(omni_lines):
        print(f"⚠️ Different number of lines: Flink has {len(flink_lines)}, Omni has {len(omni_lines)}")
