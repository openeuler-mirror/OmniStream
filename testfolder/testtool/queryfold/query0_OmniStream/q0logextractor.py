...
if __name__ == "__main__":
    log_path = find_taskexecutor_log()  # 自动找最近的日志文件

    omni_home = os.environ.get("OMNISTREAM_HOME")
    output_path = os.path.join(omni_home, "testtool/queryfold/query0_OmniStream/q0-taskname-OmniStream.txt")
...
query0_dir = os.path.join(log_home, "query0")
if not os.path.exists(query0_dir):
    os.makedirs(query0_dir)
log_filename = os.path.basename(log_file)
destination = os.path.join(query0_dir, log_filename)
shutil.copy2(log_file, destination)


omni_home = os.environ.get("OMNISTREAM_HOME")
output_file = os.path.join(omni_home, "testtool/queryfold/query0_OmniStream/q0-taskname-OmniStream.txt")
